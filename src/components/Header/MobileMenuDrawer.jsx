...
import { X, Wrench, ChevronDown, ChevronRight, Phone, MessageSquare, Heart, FileText, Globe, LayoutGrid, Zap, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex lg:hidden">
      ...
      <div className="relative w-4/5 max-w-xs bg-white h-full shadow-2xl flex flex-col justify-between z-10 animate-in slide-in-from-left duration-250">
        ...
        {/* Drawer Header */}
        <div className="p-4 bg-[#02408f] text-white flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-amber-400 text-slate-950 flex items-center justify-center font-black">
              <Wrench className="w-4 h-4 transform -rotate-12" />
            </div>
            <div>
              <h3 className="text-xs font-black uppercase tracking-wider text-white">TEJAS & COMPANY</h3>
              <span className="text-[10px] text-sky-200 font-bold block">23 Machinery Catalog</span>
            </div>
          </div>

          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="w-8 h-8 rounded-full bg-blue-950/60 hover:bg-blue-900 text-white flex items-center justify-center"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-3">
          ...
          <div className="pb-2 border-b border-slate-100 flex items-center justify-between">
            ...
            <button
              ...
              className="text-[11px] font-bold text-[#02408f] hover:underline flex items-center gap-1"
            >
              <Zap className="w-3 h-3 text-amber-500" />
              <span>Show All 23</span>
            </button>
          </div>
                    ...
                    <div className="bg-sky-50/70 p-2.5 space-y-1 text-xs border-t border-sky-100">
                      {cat.subcategories.map((sub, idx) => (
                        <button
                          ...
                          className="w-full text-left p-1.5 text-[11px] font-medium text-slate-700 hover:text-[#02408f] hover:bg-white rounded-md transition-colors block truncate"
                        >
                          • {sub}
                        </button>
                      ))}
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 space-y-2 shrink-0">
          <a
            ...
            className="w-full bg-[#02408f] text-white text-xs font-bold py-2.5 rounded-xl flex items-center justify-center gap-2 shadow-xs"
          >
          ...
          <a
            href="https://wa.me/919876543210?text=Hi%20Tejas%20%26%20Company,%20I%20want%20a%20quotation%20for%20machinery"
            target="_blank"
            rel="noreferrer"
